...
- 📱 **Apple Watch / 安卓手环震动 + 短卡片通知** — 标题点明事件类型与工具(如「权限请求 · Bash」),正文直接显示真实命令、文件路径或 Claude 的回复,一眼看清在发生什么。Bark 通知到达手机后，Apple Watch 或华为/小米/三星手环（同步手机通知）均会震动
...
每条通知都直接展示**真实发生的内容**——没有风险等级，也没有套话式的建议动作。标题点明事件类型(已知时附带工具名),一眼即可判断;正文承载实际的命令、文件路径、URL 或 Claude 的最后回复。

| 事件类型 | 标题 | 正文 |
|---------|------|------|
| 需要权限 | 权限请求 · Bash | 实际命令，如 `rm -rf build/` |
| 需要处理 | 需要处理 | Claude 的通知消息 |
| 任务完成 | 任务完成 | 从 transcript 读取的 Claude 最后回复 |
| 已拦截 | 已拦截 · Bash | 被拦截的命令 |
| 高风险 | 高风险 · Bash | 高风险命令 |
| 可能跑偏 | 可能跑偏 · 原任务 X | 触碰到的边界 |
| 连续失败 | 连续失败 · N 次 | 失败的工具内容 |
| 已拒绝 | 已拒绝 · Bash | 你拒绝的命令 |

风险/跑偏/失败检测仍然决定**是否**推送(见[通知策略](#通知策略)),只是不再决定文案措辞。
...
- **Test Permission Request / Denied / Approval Pending / Auto Exec** — 模拟特定 Hook 事件
